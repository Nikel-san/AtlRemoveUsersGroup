...
def fetch_group_members(site: str, group: str, auth: tuple) -> List[dict]:
	members: List[dict] = []
	base = f"https://{site}/rest/api/3/group/member"
	start_at = 0
	max_results = 50
	while True:
...
def fetch_org_user_status_map(org_id: str, token: str) -> Dict[str, str]:
	...
	while True:
		...
		for u in items:
			...
			acct = u.get("account_id") or u.get("accountId") or u.get("accountId") or u.get("account_id")
...
def remove_user_from_group(site: str, group: str, account_id: str, auth: tuple) -> bool:
	url = f"https://{site}/rest/api/3/group/user"
	params = {"groupname": group, "accountId": account_id}
	resp = requests.delete(url, params=params, auth=auth)
...
def main(argv: List[str] | None = None) -> int:
	...
	LOG.info("Fetching members of group '%s' on site %s", args.group, args.site)
	try:
		members = fetch_group_members(args.site, args.group, jira_auth)
	...
	try:
		status_map = fetch_org_user_status_map(org_id, admin_token)
	except Exception as exc:
	...
	for m in members:
		...
		if acct_status and acct_status.lower() != "active":
			row["action"] = "will-remove" if not args.execute else "removed"
			row["error"] = ""
			if args.execute and account_id:
				try:
					ok = remove_user_from_group(args.site, args.group, account_id, jira_auth)
					if not ok:
